...
if(common) {
	...
	let uhd = $("uhd");
	if (uhd) { //在个人主页
		common.addUserOperation("综合举报", (e, c) => {
			MCBBS.storeData("report_context", //这段内容来自版规
				`违规者用户名(必填):${document.querySelector("#uhd .mt").textContent.replace("\n","")}
违规者个人资料链接(必填):${document.querySelector("#uhd .mt").nextElementSibling.firstElementChild.href}
违规类型(必填):[签名档违规/头像违规/用户名违规/水龙头,可多选]水龙头
违规截图(可选, 水龙头举报无需附图):`);
		}, 
		`https://www.mcbbs.net/forum.php?mod=viewthread&tid=557610&page=1&authorid=${discuz_uid}`, imageurl);
	}
	...
	if (MCBBS.getData("report_context")) {
		...
		if (url == `https://www.mcbbs.net/forum.php?mod=viewthread&tid=557610&page=1&authorid=${discuz_uid}`) { //在举报专用帖下
			window.location = findEditableReportInThisPage() || "https://www.mcbbs.net/thread-557610-1-1.html";
...
${reportReason[i]}</label></td>`;
	...
	style.innerHTML = `#report_message {
		width: 97% !important
	}`;
